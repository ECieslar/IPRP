...
for m=1:columns(Temperatures)
  ...
  ## calculating the mean of regresion errors for each throw
    Par_Relative = mean(Final(:,26));
    fprintf(out_file, "Œredni b³¹d wzglêdny wyznacznia kalorycznoœci (aproksymacja p³aszczyzn¹): throw nr = %d: E=%f3,3 \n", k, Par_Relative*100);  

  endfor  
 
  ## drawing 3D picture of errors - - separate drawing for each temperature
  clear std_set
  for i=1:rows(C_table)
    std_set(i,1) = std(Final_draw(i,:));
    std_set(i,2) = mean(Final_draw(i,:));
  endfor

  std_set = [C_table(:,1:2),std_set];

  hf = figure;
    
  scatter3 (std_set(:,1), std_set(:,2), std_set(:,3),80, "r", "filled");
  hold on
  scatter3 (std_set(:,1), std_set(:,2), std_set(:,4),80, "b", "filled");
  hold off

  xlabel ("CH4");
  ylabel ("N2");
  zlabel ("Error");
  legend ("Standard deviation of error","Mean value of error");
  title  ("Calorific value errors calculated as Monte Carlo standard deviation (N=500)");
